...
def define_model(
    filter_sizes, filter_numbers, dense_expansion, windowsize, loss_function, optimizer
):
    """ "
    ...
    outX = conv_filter(
        filter_numbers[0], filter_sizes[0], strides=1, activation="relu"
    )(inputs)
    ...
    outX = Dense(dense_expansion, activation="relu")(
        outX
    )  # 'linear' units work here as well!
    outX = Flatten()(outX)
...
def calculate_noise_levels(neurons_x_time, frame_rate):
    ...
    noise_levels = np.nanmedian(np.abs(np.diff(dF_traces, axis=-1)), axis=-1) / np.sqrt(
        frame_rate
    )
...
def preprocess_traces(neurons_x_time, before_frac, window_size):
    ...
    window_indexes = (
        ...
        + np.expand_dims(np.arange(dF_traces.shape[1] - window_size + 1), 0).T
    )
    X = np.full(
        shape=(dF_traces.shape[0], dF_traces.shape[1], window_size), fill_value=np.nan
    )
    X[:, start:end, :] = dF_traces[:, window_indexes]
    return X


def calibrated_ground_truth_artificial_noise(
    ground_truth_folder,
    noise_level,
    sampling_rate,
    replicas,
    ...
    fileList = sorted(
        list(set(glob.glob(os.path.join(ground_truth_folder, "*_mini.mat"))))
    )
    ...
    for file_index, neuron_file in enumerate(fileList):
        ...
        for i, trial in enumerate(dataset_neuron_all):
            try:
                ...
                base_noise = (
                    np.nanmedian(np.abs(np.diff(traces_mean)))
                    * 100
                    / np.sqrt(frame_rate)
                )
                ...
                for test_i in np.arange(60):
                    noise_trace = np.random.normal(
                        0, test_i / 100 * np.sqrt(frame_rate), traces_mean.shape
                    )
                    test_noise[test_i] = (
                        np.nanmedian(np.abs(np.diff(noise_trace + traces_mean)))
                        * 100
                        / np.sqrt(frame_rate)
                    )

                interpolating_function = interp1d(
                    test_noise, np.arange(60), kind="linear"
                )

                if noise_level >= test_noise[0]:
                    noise_std = (
                        interpolating_function(noise_level) / 100 * np.sqrt(frame_rate)
                    )
                    ...
                    nb_subROIs = np.minimum(
                        500, np.ceil(1.2 * (noise_level / base_noise) ** 2)
                    )
                ...
                if nb_subROIs >= 1:
                    ...
                    if np.abs(sampling_rate - frame_rate) / frame_rate > 0.05:
                        num_samples = int(
                            round(traces_mean.shape[0] * sampling_rate / frame_rate)
                        )
                        (traces_mean, temp) = resample(
                            traces_mean, num_samples, np.squeeze(fluo_times), axis=0
                        )
                    ...
                    fluo_times_bin_centers = np.linspace(
                        fluo_times[0], fluo_times[-1], len(traces_mean)
                    )
                    fluo_times_bin_edges = (
                        np.append(
                            fluo_times_bin_centers,
                            fluo_times_bin_centers[-1] + 1 / sampling_rate,
                        )
                        - 1 / sampling_rate / 2
                    )

                    [events_binned, event_bins] = np.histogram(
                        events / ephys_sampling_rate, bins=fluo_times_bin_edges
                    )

                    # Limit number of replicas for longer recordings
                    nb_subROIs = int(
                        np.minimum(
                            8e5, len(dataset_neuron_all) * len(traces_mean) * nb_subROIs
                        )
                        / len(traces_mean)
                    ) / len(dataset_neuron_all)
                    ...
                    for iii in range(int(nb_subROIs)):
                        ...
                        noise_additional = np.random.normal(
                            0, noise_std * fluo_level, traces_mean.shape
                        )
                        ...
                        if np.any(sub_traces):
                            if sub_traces.shape[0] - len(sub_traces_single) >= 0:
                                sub_traces_single = np.append(
                                    sub_traces_single,
                                    np.zeros(
                                        sub_traces.shape[0] - len(sub_traces_single)
                                    )
                                    + np.nan,
                                )
                                events_binned = np.append(
                                    events_binned,
                                    np.zeros(
                                        sub_traces_events.shape[0] - len(events_binned)
                                    )
                                    + np.nan,
                                )

                            else:
                                sub_traces = np.append(
                                    sub_traces,
                                    np.zeros(
                                        (
                                            len(sub_traces_single)
                                            - sub_traces.shape[0],
                                            sub_traces.shape[1],
                                        )
                                    )
                                ...
                                sub_traces_events = np.append(
                                    sub_traces_events,
                                    np.zeros(
                                        (
                                            len(events_binned)
                                            - sub_traces_events.shape[0],
                                            sub_traces_events.shape[1],
                                        )
                                    )
                                    + np.nan,
                                    axis=0,
                            ...
                            sub_traces = np.append(
                                sub_traces, sub_traces_single.reshape(-1, 1), axis=1
                            )
                            sub_traces_events = np.append(
                                sub_traces_events, events_binned.reshape(-1, 1), axis=1
                            )
...
def preprocess_groundtruth_artificial_noise_balanced(
    ...
    dataset_sizes = np.zeros(
        len(ground_truth_folders),
    )
    ...
    for dataset_index, training_dataset in enumerate(ground_truth_folders):
        ...
        try:
            if verbose > 1:
                print("Preprocessing dataset number", dataset_index)
            ...
            (
                ...
                ground_truth_folders[dataset_index],
                noise_level,
                sampling_rate,
                replicas,
                omission_list,
            ...
            for k in range(len(sub_traces_allX)):
                try:
                    datapoint_counter += (
                        sub_traces_allX[k].shape[1] * sub_traces_allX[k].shape[0]
                    )
                except:
                    if verbose > 2:
                        print("No things for k={}".format(k))
    ...
    if verbose > 1:
        print(
            "Reducing ground truth by a factor of approximately "
            + str(int(3 * np.nanmean(reduction_factors)))
            + "."
        )
    ...
    if verbose > 1:
        print(
            "Number of neurons in the ground truth: " + str(len(sub_traces_events_all))
        )
    ...
    counter = 0
    for neuron_ix, (sub_traces, sub_traces_events) in enumerate(
        zip(sub_traces_all, sub_traces_events_all)
    ):
        if sub_traces is not None:
            for trace_index in range(sub_traces.shape[1]):
                ...
                if causal_kernel:
                    single_spikes = convolve(
                        single_spikes.astype(float),
                        causal_smoothing_kernel,
                        mode="same",
                    )

                else:
                    single_spikes = gaussian_filter(
                        single_spikes.astype(float), sigma=smoothing
                    )

                recording_length = np.sum(~np.isnan(single_trace))

                datapoints_used = np.minimum(
                    len(single_spikes) - windowsize, recording_length - windowsize
                )
                ...
                if oversampling:
                    ...
                    datapoints_used_rand = datapoints_used_rand[
                        0 : int(len(datapoints_used_rand) / (max(reduce_samples, 1)))
                    ]
                ...
                for time_points in datapoints_used_rand:
                    ...
                    X[
                        counter,
                        :,
                    ] = single_trace[time_points : (time_points + before + after)]
                    counter += 1
    ...
    if verbose > 1:
        print("Shape of training dataset X: {}    Y: {}".format(X.shape, Y.shape))
    return X, Y
...
def plot_noise_matched_ground_truth(
    model_name, median_noise, frame_rate, nb_traces, duration
):
    """
    ...
    if (
        ...
        training_folders = [
            w.replace("Ground_truth/DS02-Cal520-m-S1", "Ground_truth/DS03-Cal520-m-S1")
            for w in training_folders
        ]
        training_folders = [
            w.replace("Ground_truth/DS03-OGB1-zf-pDp", "Ground_truth/DS04-OGB1-zf-pDp")
            for w in training_folders
        ]
        training_folders = [
            w.replace(
        ...
        training_folders = [
            w.replace(
                "Ground_truth/DS14-GCaMP6s-m-V1",
                "Ground_truth/DS15-GCaMP6s-m-V1",
            )
            for w in training_folders
        ]
        training_folders = [
        ...
        training_folders = [
            w.replace(
                "Ground_truth/DS10-GCaMP6f-m-V1-neuropil-corrected",
                "Ground_truth/DS11-GCaMP6f-m-V1-neuropil-corrected",
            )
            for w in training_folders
        ]
        training_folders = [
            w.replace(
                "Ground_truth/DS09-GCaMP6f-m-V1-neuropil-corrected",
                "Ground_truth/DS10-GCaMP6f-m-V1-neuropil-corrected",
            )
            for w in training_folders
        ]
        training_folders = [
            w.replace(
                "Ground_truth/DS12-GCaMP6s-m-V1-neuropil-corrected",
                "Ground_truth/DS13-GCaMP6s-m-V1-neuropil-corrected",
            )
            for w in training_folders
        ]
        training_folders = [
            w.replace(
                "Ground_truth/DS11-GCaMP6s-m-V1-neuropil-corrected",
                "Ground_truth/DS12-GCaMP6s-m-V1-neuropil-corrected",
            )
            for w in training_folders
        ]
        ...
        training_folders = [
            w.replace("Ground_truth/DS18-R-CaMP-m-S1", "Ground_truth/DS19-R-CaMP-m-S1")
            for w in training_folders
        ]
        training_folders = [
            w.replace(
